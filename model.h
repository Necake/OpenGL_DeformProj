...
private:
	...
	void loadModel(string const& path, bool isDynamic)
	{
		...
		const aiScene* scene = importer.ReadFile(path, aiProcess_FlipUVs | aiProcess_CalcTangentSpace | aiProcess_JoinIdenticalVertices);
		// check for errors
	...
	Mesh processMesh(aiMesh * mesh, const aiScene * scene, bool isDynamic)
	{
		...
		this->material.diffuse = glm::vec3(color.r, color.g, color.b);

		material->Get(AI_MATKEY_COLOR_AMBIENT, color);
		this->material.ambient = glm::vec3(color.r, color.g, color.b);

		material->Get(AI_MATKEY_COLOR_SPECULAR, color);
		this->material.specular = glm::vec3(color.r, color.g, color.b);
